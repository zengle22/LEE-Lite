...
1. **How does validate_output.sh get the FRZ reference?** (RESOLVED)
   - silent_override.py extracts frz_ref from output artifacts (e.g., feat-package.json), falls back to `--frz` CLI parameter
   - validate_output.sh calls: `python cli/lib/silent_override.py check --output <output-dir> --frz <frz-id>`

2. **What is the exact JSON structure of dimension_reviews?** (RESOLVED)
   - Each dimension value follows the pattern of existing dimensions: includes `verdict` ("pass"/"block"/"pass_with_revisions") and `semantic_drift` field (D-06)

3. **Does ll-dev-feat-to-surface-map need validate_output.sh created?** (RESOLVED)
   - Yes, create validate_output.sh following the standard pattern during 09-03 plan execution
