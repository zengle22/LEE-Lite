...
class SrcToEpicWorkflowTests(unittest.TestCase):
    ...
    def test_governance_runtime_src_keeps_single_epic_with_adoption_e2e_requirements(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertTrue(any(item.startswith("技能接入与跨 skill 闭环验证能力：") for item in epic["scope"]))
    ...
    def test_governance_bridge_epic_demotes_qa_object_rules_to_inherited_constraints(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertIn("统一上位能力：形成一条可被多 skill 共享继承的主链受治理交接闭环。", epic["scope"])
            self.assertTrue(any("primary decomposition axis" in item for item in epic["decomposition_rules"]))
            self.assertTrue(any("mandatory cross-cutting overlays" in item for item in epic["decomposition_rules"]))
            self.assertTrue(any("producer -> consumer -> audit -> gate" in item for item in epic["success_metrics"]))
            self.assertTrue(any("candidate -> formal materialization" in item for item in epic["success_metrics"]))
