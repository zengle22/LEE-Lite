...
class EpicToFeatWorkflowTests(unittest.TestCase):
    ...
    def test_standard_epic_keeps_foundation_feats_only(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            integration_context = json.loads((artifacts_dir / "integration-context.json").read_text(encoding="utf-8"))
            ...
            self.assertIn("integration_context", bundle)
            self.assertIn("prohibited_inference_rules", bundle)
            self.assertEqual(bundle["integration_context"], integration_context)
            self.assertTrue(bundle["glossary"])
            self.assertTrue(bundle["prohibited_inference_rules"])
            self.assertEqual(handoff["integration_context_ref"], "integration-context.json")
            ...
            self.assertEqual(package_manifest["integration_context_ref"], str(artifacts_dir / "integration-context.json"))
