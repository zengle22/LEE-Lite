...
class GateHumanOrchestratorWorkflowTests(GateHumanOrchestratorTestSupport):
    ...
    def test_prepare_round_tech_design_package_renders_human_friendly_implementation_summary(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertTrue(any(item.startswith("归属映射:") for item in summary["ssot_excerpt"]))
            self.assertTrue(any(item.startswith("TECH owner:") for item in summary["ssot_excerpt"]))
            ...
            self.assertTrue(any("design ownership:" in item for item in summary["ssot_outline"]))
            ...
            self.assertTrue(any("surface-map" in item for item in summary["review_checkpoints"]))
            self.assertIn("### 设计归属与权威引用", markdown)
            self.assertIn("SURFACE-MAP-FEAT-SRC-005-001", markdown)
            self.assertIn("TECH-SRC-005-001", markdown)
    ...
    def test_prepare_round_impl_package_renders_human_friendly_delivery_summary(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertTrue(any(item.startswith("实现目标:") for item in summary["ssot_excerpt"]))
            self.assertTrue(any(item.startswith("归属映射:") for item in summary["ssot_excerpt"]))
            self.assertTrue(any(item.startswith("设计引用:") for item in summary["ssot_excerpt"]))
            ...
            self.assertTrue(any("surface_map_ref" in item or "prototype_ref" in item for item in summary["review_checkpoints"]))
            self.assertIn("### 设计归属与已解析引用", markdown)
            self.assertIn("SURFACE-MAP-FEAT-SRC-005-001", markdown)
            self.assertIn("PROTO-SRC-005-001", markdown)
            self.assertIn("UI-SRC-005-001", markdown)
