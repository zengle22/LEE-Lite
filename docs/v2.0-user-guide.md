...
```bash
ll frz-manage validate --input <doc-dir>
```

**输入**: 包含 FRZ YAML 和相关产品文档的目录  
...
```bash
ll frz-manage freeze --input <doc-dir> --id FRZ-001
```
...
# 只看已冻结的
ll frz-manage list --status frozen
```
...
> 使用技能: `ll-product-raw-to-src`

```bash
ll product-raw-to-src extract --frz FRZ-001 --output <src-dir>
```
...
> 使用技能: `ll-product-src-to-epic`

```bash
# 步骤 1: Executor 运行
python scripts/src_to_epic.py executor-run --input <src-package-dir> --repo-root .

# 步骤 2: Supervisor 审查
python scripts/src_to_epic.py supervisor-review --artifacts-dir <epic-package-dir>

# 步骤 3: 冻结守卫（语义通过后才能冻结）
python scripts/src_to_epic.py freeze-guard --artifacts-dir <epic-package-dir>
```
...
> 使用技能: `ll-product-epic-to-feat`

```bash
# 步骤 1: Executor 运行
python scripts/epic_to_feat.py executor-run --input <epic-package-dir> --repo-root .

# 步骤 2: Supervisor 审查
python scripts/epic_to_feat.py supervisor-review --artifacts-dir <feat-package-dir>

# 步骤 3: 冻结守卫
python scripts/epic_to_feat.py freeze-guard --artifacts-dir <feat-package-dir>
```

**关键规则**:
- 只接受从 `ll-product-src-to-epic` 产出的冻结就绪的 EPIC 包
- 不接受原始需求文档或 ADR 文本
...
```bash
ll product-raw-to-src --input <doc-path>
```
...
### 5.3 Minor 变更处理（ll-experience-patch-settle）

Minor Patch 验证通过后，自动执行 settlement：

```bash
# 上游 ll-patch-capture 已验证并批准的 Minor Patch 会自动进入 settlement
# 也可以手动指定 Patch 文件
ll experience-patch-settle --input <patch-yaml>
```
...
**重要**: 回写 RECORD 是给人类审核的结构化记录，**不会直接修改 SSOT 文件**。如果需要实际修改 SSOT，需要人工确认后使用 `--apply` 标志（未来版本）。

### 5.4 Major 变更处理（ll-frz-manage --type revise）

Major Patch 不能走 settlement，必须回流 FRZ：

```bash
ll frz-manage freeze --type revise --input <new-doc-dir> --previous-frz FRZ-001 --reason "新增 XX 业务规则"
```
...
**与普通 freeze 的区别**：
- 多了 `--previous-frz` 和 `--reason` 参数（必须提供）
...
你需要手动重跑：
```
ll product-raw-to-src extract --frz FRZ-002
SRC → EPIC → FEAT（executor-run → supervisor-review → freeze-guard）
```
...
在开始写代码之前，运行 `ll-qa-impl-spec-test` 对实施准备情况做压力测试。

### 6.1 运行门禁检查

```bash
python -m cli skill impl-spec-test --request <request.json> --response-out <response.json>
```
...
# Python 验证
python -c "
from cli.lib.task_pack_schema import validate_file
result = validate_file('ssot/tasks/PACK-SRC-001-001-feat001.yaml')
print(f'Pack: {result.pack_id}, Tasks: {len(result.tasks)}')
"
```
...
```bash
# 解析依赖，返回可执行顺序
python -m cli.lib.task_pack_resolver ssot/tasks/PACK-SRC-001-001-feat001.yaml
```
...
1. 解析依赖顺序：`python -m cli.lib.task_pack_resolver <pack.yaml>`
2. 按输出顺序逐一执行 task
...
```
1. 准备产品文档（PRD、UX、架构）
2. ll frz-manage validate --input <doc-dir>     # MSC 验证
3. ll frz-manage freeze --input <doc-dir> --id FRZ-001  # 冻结
4. ll product-raw-to-src extract --frz FRZ-001    # 抽取 SRC
...
3. Supervisor 验证通过
4. ll experience-patch-settle 回写 RECORD
...
3. ll frz-manage freeze --type revise --previous-frz FRZ-001
...
2. python -m cli skill impl-spec-test --request request.json --response-out response.json
3. 查看 verdict：
   - pass → 开始编码
   - block → 先解决问题
```
...
skills/
├── ll-frz-manage/                # FRZ 生命周期管理技能
├── ll-product-src-to-epic/       # SRC → EPIC 抽取技能
├── ll-product-epic-to-feat/      # EPIC → FEAT 抽取技能
├── ll-patch-capture/             # 变更捕获 + 三分类技能
├── ll-experience-patch-settle/   # Minor Patch settlement 技能
└── ll-qa-impl-spec-test/         # 实施前质量门禁技能
```
