...
3. Produce `source-semantic-findings.json` and `acceptance-report.json` as distinct artifacts without mutating the candidate directly.
4. Record `supervision-evidence.json` with a recommendation such as `pass` or `revise`.
5. Recommend whether the package appears `freeze_ready`, needs `retry`, needs `human_handoff`, or should remain `blocked`, while leaving final routing to the external gate.
