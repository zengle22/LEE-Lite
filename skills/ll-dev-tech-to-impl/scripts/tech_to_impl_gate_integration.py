#!/usr/bin/env python3
"""
Gate pending integration helpers for tech-to-impl.
"""
...
def create_handoff_proposal(
    ...
    _write_json(
        proposal_path,
        {
            ...
            "tech_ref": tech_ref,
            "impl_ref": impl_ref,
            "primary_artifact_ref": _repo_relative(repo_root, artifacts_dir / "impl-bundle.md"),
            "supporting_artifact_refs": [
                _repo_relative(repo_root, artifacts_dir / "impl-bundle.json"),
                _repo_relative(repo_root, artifacts_dir / "impl-task.md"),
                _repo_relative(repo_root, artifacts_dir / "impl-review-report.json"),
                _repo_relative(repo_root, artifacts_dir / "impl-acceptance-report.json"),
                _repo_relative(repo_root, artifacts_dir / "impl-defect-list.json"),
                _repo_relative(repo_root, artifacts_dir / "handoff-to-feature-delivery.json"),
            ],
            "required_context": [
                "selected upstream feat and tech refs",
                "implementation task package",
                "smoke gate execution readiness",
                "downstream delivery handoff",
            ],
            "expected_output_type": "gate_decision_package",
            "status_reason": "Awaiting external gate review before IMPL formalization or downstream execution admission.",
        },
    )
...
def submit_gate_pending(
    ...
    request = {
        ...
        "request_id": f"req-tech-to-impl-{run_id}-gate-submit",
        "workspace_root": repo_root.as_posix(),
        "actor_ref": "ll-dev-tech-to-impl",
        "trace": {
            "run_ref": run_id,
            "workflow_key": "dev.tech-to-impl",
        },
        "payload": {
            "producer_ref": "ll-dev-tech-to-impl",
    ...
    if exit_code != 0 or response.get("status_code") != "OK":
        raise RuntimeError(f"tech-to-impl gate submit failed: {response.get('status_code')} {response.get('message')}")
    return {
