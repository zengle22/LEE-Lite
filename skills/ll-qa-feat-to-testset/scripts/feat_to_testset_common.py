#!/usr/bin/env python3
"""
Shared helpers for the lite-native feat-to-testset runtime.
"""
...
def derive_test_set_ref(feat_ref: str) -> str:
    ...
    if feat_ref.startswith("FEAT-"):
        return feat_ref.replace("FEAT-", "TESTSET-", 1)
    return f"TESTSET-{slugify(feat_ref).upper()}"


def derive_test_set_id(feat_ref: str) -> str:
    feat_ref = feat_ref.strip()
    if feat_ref.startswith("FEAT-"):
        return feat_ref.replace("FEAT-", "TS-", 1)
    return f"TS-{slugify(feat_ref).upper()}"
...
def validate_input_package(artifacts_dir: Path, feat_ref: str) -> tuple[list[str], dict[str, Any]]:
    ...
    if "TESTSET" not in derivable_children:
        errors.append("handoff-to-feat-downstreams.json must include TESTSET in derivable_children.")
