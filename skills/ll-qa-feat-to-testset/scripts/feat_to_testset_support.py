...
def derive_environment_assumptions(
    ...
    extras: dict[str, list[str]] = {
        ...
        "minimal_onboarding": [
            "登录/注册后进入最小建档页、提交成功进入首页、设备连接后置这三段产品流必须可重复执行。",
            "字段级校验错误、homepage entry allowance 与 deferred device entry 状态必须可观测。",
        ],
        "first_ai_advice": [
            "首轮建议最低输出字段与 risk gate 判定必须可观测。",
            "扩展画像或设备数据缺失时的 conservative prompt / follow-up prompt 路径必须可重复验证。",
        ],
        "extended_profile_completion": [
            "首页任务卡、增量保存与完成度刷新必须可重复执行。",
            "patch save failure 必须保留 homepage 可用与 retry entry。",
        ],
        "device_deferred_entry": [
            "首页后置设备入口、skip 路径与 non-blocking failure 结果必须可重复验证。",
            "设备增强结果不得反向影响首页进入或首轮建议可用性。",
        ],
        "state_profile_boundary": [
            "primary_state / capability_flags 与 canonical body facts 的边界必须可重复验证。",
            "cross-boundary conflict 与 unified-reader fail-closed 结果必须可观测。",
        ],
        "formal": [
...
def derive_risk_focus(feature: dict[str, Any], profile: str) -> list[str]:
    ...
    extras: dict[str, list[str]] = {
        ...
        "minimal_onboarding": ["最小建档字段缺失仍放行首页", "设备连接重新变成首进阻塞前置"],
        "first_ai_advice": ["风险门槛字段缺失仍生成正常建议", "首轮建议最低输出字段不完整"],
        "extended_profile_completion": ["patch 保存失败回撤首页可用性", "任务卡增量保存后 completion percent 不更新"],
        "device_deferred_entry": ["设备连接失败阻塞首页或首轮建议", "设备数据回写覆盖 canonical onboarding/profile 事实"],
        "state_profile_boundary": ["primary_state 与 capability_flags 混写", "non-canonical store 覆盖 user_physical_profile"],
