    ...
    return [
        ...
        unit_payload(
            ...
            [
                ...
                "error code -> retryable -> idempotent_replay mapping",
            ],
            [
                "下游直接继承主链 queue / handoff / gate 语义。",
                "没有并行 handoff model 或额外 queue。",
                "error code、retryable、idempotent_replay 映射在下游消费链中保持一致。",
            ],
            [
                "下游重新定义 handoff 规则。",
                "出现并行 queue 或重写 re-entry 语义。",
                "retryable 或 idempotent_replay 语义与主链 envelope 漂移。",
            ],
            [
                "downstream integration evidence",
                "queue inheritance trace",
                "handoff contract comparison",
                "error mapping comparison",
            ],
