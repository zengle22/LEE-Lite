#!/usr/bin/env python3
"""
Gate pending integration helpers for src-to-epic.
"""
...
def create_handoff_proposal(
    ...
    _write_json(
        proposal_path,
        {
            ...
            "primary_artifact_ref": _repo_relative(repo_root, artifacts_dir / "epic-freeze.md"),
            "supporting_artifact_refs": [
                _repo_relative(repo_root, artifacts_dir / "epic-freeze.json"),
                _repo_relative(repo_root, artifacts_dir / "epic-review-report.json"),
                _repo_relative(repo_root, artifacts_dir / "epic-acceptance-report.json"),
                _repo_relative(repo_root, artifacts_dir / "epic-defect-list.json"),
                _repo_relative(repo_root, artifacts_dir / "handoff-to-epic-to-feat.json"),
            ],
            "required_context": [
                "epic intent and business goal",
                "decomposition rules",
                "constraint groups",
                "traceability",
            ],
            "expected_output_type": "gate_decision_package",
            "status_reason": "Awaiting external gate review before epic formalization and downstream admission.",
        },
    )
...
def submit_gate_pending(
    ...
    request = {
        ...
        "request_id": f"req-src-to-epic-{run_id}-gate-submit",
        ...
        "payload": {
            "producer_ref": "ll-product-src-to-epic",
    ...
    if exit_code != 0 or response.get("status_code") != "OK":
        raise RuntimeError(f"src-to-epic gate submit failed: {response.get('status_code')} {response.get('message')}")
    return {
