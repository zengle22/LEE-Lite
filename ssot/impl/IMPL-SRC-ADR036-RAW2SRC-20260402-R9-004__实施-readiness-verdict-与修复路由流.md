---
id: "IMPL-SRC-ADR036-RAW2SRC-20260402-R9-004"
ssot_type: IMPL
title: 实施 readiness verdict 与修复路由流
status: execution_ready
schema_version: 1.0.0
workflow_key: "dev.tech-to-impl"
workflow_run_id: "adr036-feat004-impl-r1"
source_refs:
- "dev.feat-to-tech::adr036-src2epic-20260402-r4--feat-src-adr036-raw2src-20260402-r9-004"
...
- "API-SRC-ADR036-RAW2SRC-20260402-R9-004"
candidate_artifact_ref: "artifacts/tech-to-impl/adr036-feat004-impl-r1/impl-task.md"
...
tech_ref: "TECH-SRC-ADR036-RAW2SRC-20260402-R9-004"
arch_ref: "ARCH-SRC-ADR036-RAW2SRC-20260402-R9-004"
api_ref: "API-SRC-ADR036-RAW2SRC-20260402-R9-004"
candidate_package_ref: "artifacts/tech-to-impl/adr036-feat004-impl-r1"
---

# IMPL-SRC-ADR036-RAW2SRC-20260402-R9-004

## 1. 任务标识

- impl_ref: `IMPL-SRC-ADR036-RAW2SRC-20260402-R9-004`
- title: 实施 readiness verdict 与修复路由流 Implementation Task Package
- workflow_key: `dev.tech-to-impl`
- workflow_run_id: `adr036-feat004-impl-r1`
- status: `execution_ready`
- derived_from: `FEAT-SRC-ADR036-RAW2SRC-20260402-R9-004`, `TECH-SRC-ADR036-RAW2SRC-20260402-R9-004`
- package role: canonical execution package / execution-time single entrypoint

## 2. 本次目标

- 覆盖目标: 冻结 score-to-verdict、blocking/high-priority issue 和 repair_target_artifact 的输出语义。
...
- SRC / EPIC / FEAT: `SRC-ADR036-RAW2SRC-20260402-R9` / `EPIC-IMPL-IMPLEMENTATION-READINESS` / `FEAT-SRC-ADR036-RAW2SRC-20260402-R9-004` -> 冻结 score-to-verdict、blocking/high-priority issue 和 repair_target_artifact 的输出语义。
- TECH: `TECH-SRC-ADR036-RAW2SRC-20260402-R9-004` -> Freeze a concrete TECH design for 实施 readiness verdict 与修复路由流, preserving FEAT semantics while making runtime carriers and contracts implementation-ready.; 来源与依赖约束：`IMPL` 是主测试对象，`FEAT / TECH / ARCH / API / UI / TESTSET` 是联动 authority。; 来源与依赖约束：authoritative upstream objects 冲突时，workflow 只能检测、升级并建议修复目标，不得自行建立新的 business truth 或 design truth。
- ARCH: `ARCH-SRC-ADR036-RAW2SRC-20260402-R9-004` -> Architecture boundaries constrain layering, ownership, and runtime attachment points.
- API: `API-SRC-ADR036-RAW2SRC-20260402-R9-004` -> `genericRequest`: freeze a machine-readable request/response contract before implementation.; API contract changes remain governed by upstream API truth.
- UI: `missing_authority` -> No explicit UI ref selected and no accepted UI authority was discoverable for `FEAT-SRC-ADR036-RAW2SRC-20260402-R9-004` (expected `UI-FEAT-SRC-ADR036-RAW2SRC-20260402-R9-004`). Treat this as a controlled authority gap; coder may follow only embedded UI entry/exit constraints until UI authority is frozen or revised.
- TESTSET: `missing_authority` -> No explicit TESTSET ref selected and no accepted TESTSET authority was discoverable for `FEAT-SRC-ADR036-RAW2SRC-20260402-R9-004` (expected `TESTSET-SRC-ADR036-RAW2SRC-20260402-R9-004`). Acceptance trace is only a temporary execution proxy; freeze or revise TESTSET authority before final execution/signoff.
...
- `ARCH` status=`bound` ref=`ARCH-SRC-ADR036-RAW2SRC-20260402-R9-004` | required_for: layering and ownership constraints when ARCH applies | execution_effect: IMPL inherits architecture boundaries only when ARCH was selected upstream | follow_up: none
- `API` status=`bound` ref=`API-SRC-ADR036-RAW2SRC-20260402-R9-004` | required_for: interface contract snapshots and response invariants when API applies | execution_effect: IMPL inherits API truth only when API was selected upstream | follow_up: none
- `UI` status=`missing` ref=`UI-FEAT-SRC-ADR036-RAW2SRC-20260402-R9-004` | required_for: UI entry/exit constraints and user-facing acceptance wording | execution_effect: coder may rely on embedded UI contract only within the declared IMPL boundary | follow_up: freeze_or_revise_ui_before_final_execution
- `TESTSET` status=`missing` ref=`TESTSET-SRC-ADR036-RAW2SRC-20260402-R9-004` | required_for: acceptance truth, evidence collection, and tester alignment | execution_effect: acceptance trace remains a proxy until TESTSET authority is frozen | follow_up: freeze_or_revise_testset_before_final_execution
...
- Freeze a concrete TECH design for 实施 readiness verdict 与修复路由流, preserving FEAT semantics while making runtime carriers and contracts implementation-ready.
...
- 实施 readiness verdict 与修复路由流 必须保持为独立可验收的产品切片，不能退化为页面字段清单、接口清单或实现任务。
- 实施 readiness verdict 与修复路由流 的完成态必须与“implementation consumer 无需回读 ADR 即可知道能否开工、哪里要修、由谁修。”对齐，不能只输出中间态、占位态或内部处理结果。
- 实施 readiness verdict 与修复路由流 happy path reaches the declared completed state: implementation consumer 无需回读 ADR 即可知道能否开工、哪里要修、由谁修。
- 实施 readiness verdict 与修复路由流 keeps its declared product boundary: 该 FEAT 只覆盖“dimension score、pass/pass_with_revisions/block verdict、repair target、missing information、repair plan。”及其直接完成结果，不吸收相邻产品切片、实现任务或测试执行细节。
- 实施 readiness verdict 与修复路由流 hands downstream one authoritative product deliverable: 下游必须围绕 implementation-readiness verdict package 继承该 FEAT 的产品语义，而不是重新猜测完成条件、补写边界或改写验收口径。
...
- Boundary guardrail: dimension score、pass/pass_with_revisions/block verdict、repair target、missing information、repair plan。
- Boundary guardrail: 冻结 实施 readiness verdict 与修复路由流 这一独立产品行为切片，并把它保持在产品层边界内。
