---
id: "IMPL-SRC-ADR036-RAW2SRC-20260402-R9-002"
ssot_type: IMPL
title: 跨文档一致性与产品行为边界评审流
status: execution_ready
schema_version: 1.0.0
workflow_key: "dev.tech-to-impl"
workflow_run_id: "adr036-feat002-impl-r1"
source_refs:
- "dev.feat-to-tech::adr036-src2epic-20260402-r4--feat-src-adr036-raw2src-20260402-r9-002"
...
candidate_artifact_ref: "artifacts/tech-to-impl/adr036-feat002-impl-r1/impl-task.md"
...
tech_ref: "TECH-SRC-ADR036-RAW2SRC-20260402-R9-002"
arch_ref: "ARCH-SRC-ADR036-RAW2SRC-20260402-R9-002"
api_ref: "API-SRC-ADR036-RAW2SRC-20260402-R9-002"
candidate_package_ref: "artifacts/tech-to-impl/adr036-feat002-impl-r1"
---

# IMPL-SRC-ADR036-RAW2SRC-20260402-R9-002

## 1. 任务标识

- impl_ref: `IMPL-SRC-ADR036-RAW2SRC-20260402-R9-002`
- title: 跨文档一致性与产品行为边界评审流 Implementation Task Package
- workflow_key: `dev.tech-to-impl`
- workflow_run_id: `adr036-feat002-impl-r1`
- status: `execution_ready`
- derived_from: `FEAT-SRC-ADR036-RAW2SRC-20260402-R9-002`, `TECH-SRC-ADR036-RAW2SRC-20260402-R9-002`
- package role: canonical execution package / execution-time single entrypoint

## 2. 本次目标

- 覆盖目标: 冻结 IMPL 与联动 authority 之间的功能逻辑、状态、API、UI、旅程和测试可观测性检查。
...
- SRC / EPIC / FEAT: `SRC-ADR036-RAW2SRC-20260402-R9` / `EPIC-IMPL-IMPLEMENTATION-READINESS` / `FEAT-SRC-ADR036-RAW2SRC-20260402-R9-002` -> 冻结 IMPL 与联动 authority 之间的功能逻辑、状态、API、UI、旅程和测试可观测性检查。
- TECH: `TECH-SRC-ADR036-RAW2SRC-20260402-R9-002` -> Freeze a concrete TECH design for 跨文档一致性与产品行为边界评审流, preserving FEAT semantics while making runtime carriers and contracts implementation-ready.; 来源与依赖约束：`IMPL` 是主测试对象，`FEAT / TECH / ARCH / API / UI / TESTSET` 是联动 authority。; 来源与依赖约束：authoritative upstream objects 冲突时，workflow 只能检测、升级并建议修复目标，不得自行建立新的 business truth 或 design truth。
- ARCH: `ARCH-SRC-ADR036-RAW2SRC-20260402-R9-002` -> Architecture boundaries constrain layering, ownership, and runtime attachment points.
- API: `API-SRC-ADR036-RAW2SRC-20260402-R9-002` -> `genericRequest`: freeze a machine-readable request/response contract before implementation.; API contract changes remain governed by upstream API truth.
- UI: `missing_authority` -> No explicit UI ref selected and no accepted UI authority was discoverable for `FEAT-SRC-ADR036-RAW2SRC-20260402-R9-002` (expected `UI-FEAT-SRC-ADR036-RAW2SRC-20260402-R9-002`). Treat this as a controlled authority gap; coder may follow only embedded UI entry/exit constraints until UI authority is frozen or revised.
- TESTSET: `missing_authority` -> No explicit TESTSET ref selected and no accepted TESTSET authority was discoverable for `FEAT-SRC-ADR036-RAW2SRC-20260402-R9-002` (expected `TESTSET-SRC-ADR036-RAW2SRC-20260402-R9-002`). Acceptance trace is only a temporary execution proxy; freeze or revise TESTSET authority before final execution/signoff.
...
- `ARCH` status=`bound` ref=`ARCH-SRC-ADR036-RAW2SRC-20260402-R9-002` | required_for: layering and ownership constraints when ARCH applies | execution_effect: IMPL inherits architecture boundaries only when ARCH was selected upstream | follow_up: none
- `API` status=`bound` ref=`API-SRC-ADR036-RAW2SRC-20260402-R9-002` | required_for: interface contract snapshots and response invariants when API applies | execution_effect: IMPL inherits API truth only when API was selected upstream | follow_up: none
- `UI` status=`missing` ref=`UI-FEAT-SRC-ADR036-RAW2SRC-20260402-R9-002` | required_for: UI entry/exit constraints and user-facing acceptance wording | execution_effect: coder may rely on embedded UI contract only within the declared IMPL boundary | follow_up: freeze_or_revise_ui_before_final_execution
- `TESTSET` status=`missing` ref=`TESTSET-SRC-ADR036-RAW2SRC-20260402-R9-002` | required_for: acceptance truth, evidence collection, and tester alignment | execution_effect: acceptance trace remains a proxy until TESTSET authority is frozen | follow_up: freeze_or_revise_testset_before_final_execution
...
- Boundary guardrail: functional logic、state/data、user journey、UI、API、testability、migration compatibility 多维一致性评审。
- Boundary guardrail: 冻结 跨文档一致性与产品行为边界评审流 这一独立产品行为切片，并把它保持在产品层边界内。
