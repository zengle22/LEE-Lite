...
workflow_run_id: adr036-raw2src-20260402-r2
source_kind: governance_bridge_src
source_refs:
  - ADR-036
candidate_package_ref: artifacts/raw-to-src/adr036-raw2src-20260402-r2
---
...
- AI coder / tester / reviewer 从同一组文档中读出不同 truth，导致实现偏离产品行为主语。
...
- Actors: Dev workflow / orchestration 设计者; `ll-dev-tech-to-impl` 与下游 implementation runtime 作者; human gate / reviewer; AI coder / tester / implementation consumer
- Product surfaces: `qa.impl-spec-test` workflow; `ll-qa-impl-spec-test`; `feature_impl_candidate_package`; `impl_spec_test_report_package`; `implementation_readiness_gate_subject`; `pass / pass_with_revisions / block` verdict
...
- Runtime objects: `feature_impl_candidate_package`; `impl_spec_test_report_package`; `implementation_readiness_gate_subject`; counterexample coverage record
...
- 治理对象：`qa.impl-spec-test`; `ll-qa-impl-spec-test`; implementation-readiness verdict; repair target routing; counterexample coverage rules
...
- Source refs: ADR-036
- Input type: adr
- Canonical raw-to-src candidate: `artifacts/raw-to-src/adr036-raw2src-20260402-r2`
